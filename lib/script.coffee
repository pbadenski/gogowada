...
class Chart
  ...
  configure: (onSuccess = () -> null) ->
    ...
    fieldDimension = @csData.dimension(@dimension())
    ...
    chart
      ...
      .turnOnControls(true)
      .on "postRender", (chart) ->
         gridster_cols = Math.ceil(self.gridWidget.find('.dc-chart').width() / self.gridster.options.widget_base_dimensions[0])
         gridster_rows = Math.ceil(self.gridWidget.find('.dc-chart').height() / self.gridster.options.widget_base_dimensions[1])
